...
class LSTMTagger(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size,n_layers, bidirectional):
        super(LSTMTagger, self).__init__()
        self.hidden_dim = hidden_dim
        self.n_layers = n_layers
        self.bidirectional = bidirectional
        self.word_embeddings = nn.Embedding(vocab_size, embedding_dim)
     #   self.word_embeddings = preroll
    ...
    #    lstm_out, hidden = self.lstm(embeds, hidden)
        lstm_out, hidden = self.lstm(embeds.view(batch_size, len(sentence[0]), embedding_dim),hidden)
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (weight.new(self.n_layers*number, batch_size, self.hidden_dim).zero_().to(device),
...
net = LSTMTagger(embedding_dim, hidden_dim, vocab_size, 1,n_layers,bidirectional).to(device) 
lr = 0.001
